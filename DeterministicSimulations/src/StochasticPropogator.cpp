...
double Driver(double ProtDuration, double MeanDistance, int OuterIterations,int InnerIterations, double VelVar, double MeanVel, double * OptVel, double * CPVals, int PeriodLength){
	...
	//double * PosTracker;
	//double * TimeTracker;
	//CPTrajectory = new double [ProtocolLength];
	//PosTracker = new double [ProtocolLength];
	//TimeTracker = new double [ProtocolLength];
	//int CP_Counter = 0;
	...
	for(int k = 0 ; k < OuterIterations ; k++){
		...
		//double * PosTracker;
		//double * TimeTracker;
		//CPTrajectory = new double [500000];
		//PosTracker = new double [500000];
		//TimeTracker = new double [500000];
		//int CP_Counter = 0;
		...
		for(int j = 0 ; j < InnerIterations ; j++){
			...
			while(time < Equilibration){
				...
				//PosTracker[CP_Counter] = position;
				//CP_Counter += 1;
			...
			while(time < ProtDuration){
				...
				//CPTrajectory[CP_Counter] = CP;
				//PosTracker[CP_Counter] = position;
				//TimeTracker[CP_Counter] = time;
				//CP_Counter += 1;
			}

		}

		//WriteName = "TrajData/CPTraj_T" + std::to_string(ProtDuration) + "_" + std::to_string(k) + ".dat";
		//std::ofstream WriteFile;
		//WriteFile.open(WriteName);
		//for(int c = 0 ; c < 100000 ; c++){
		//	WriteFile << std::to_string(TimeTracker[c]) << "\t" << std::to_string(CPTrajectory[c]) << "\n";
		//}
		//WriteFile.close();

		//WriteName = "TrajData/PosTraj_T" + std::to_string(ProtDuration) + "_" + std::to_string(k) + ".dat";
		//std::ofstream WriteFile2;
		//WriteFile2.open(WriteName);
		//for(int c = 0 ; c < 100000 ; c++){
		//	WriteFile2 << std::to_string(TimeTracker[c]) << "\t" << std::to_string(PosTracker[c]) << "\n";
		//}
		//WriteFile2.close();
